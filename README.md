...
- 本项目仅用于个人求职资料整理、辅助填写与投递记录管理，不构成就业、法律、隐私、安全或任何其他专业建议，也不保证获得面试、录用或任何招聘结果。
- 使用者应自行核对简历、网页填写内容、附件和投递信息的真实性、完整性及适用性，并自行完成最终提交、上传、授权和确认。因填写错误、遗漏、过期信息或第三方网站变更造成的后果由使用者自行承担。
- 使用者必须遵守所在地区法律法规，以及各招聘网站、公司和服务商的使用条款、隐私政策与反自动化规则；不得将本工具用于规避验证码、绕过访问限制、批量骚扰、虚假投递或其他违法违规用途。
- AI 输出和字段匹配仅为待审核建议，可能存在错误、遗漏或不适用于具体岗位的情况。请勿在未审核的情况下直接依赖其结果。
- 资料、浏览器登录状态和 API Key 默认保存在本机。配置模型功能时，相关文本会发送至使用者自行选择的模型服务商；使用者应自行评估该服务商的安全性、数据处理规则与费用，并避免提交不必要的敏感信息。
- 本项目按“现状”提供。在法律允许的范围内，开发者不对使用本项目导致的直接或间接损失承担责任。
...
下面的步骤适用于 EXE 和 Python 源码版，二者的界面相同。

1. 在你所使用的模型服务商官网创建或复制一个 API Key。这个 Key 相当于密码，只应粘贴到本机程序的“模型设置”中。
2. 打开“投了么”，在左侧进入“模型设置”。
3. 在服务商的 API 文档或控制台找到两项内容：兼容 OpenAI 的接口地址，以及模型 ID。
4. 将接口地址粘到“接口地址”；填写完整的 `chat/completions` 地址最稳妥。若服务商只给 Base URL，程序也会自动尝试补上该路径。
5. 将模型 ID 粘到“模型名称”。模型 ID 必须与服务商控制台中的拼写完全一致。
6. 将 API Key 粘到“API Key”。输入框会隐藏字符；不要把 Key 发给他人或贴到 GitHub。
7. 点击“保存模型设置”，然后点击“测试模型连接”。看到连接成功提示后才开始导入简历或使用 AI 对话。

以硅基流动为例：在其控制台创建 API Key 后，通常填写其 OpenAI 兼容接口地址；模型名称填写你账户实际开通的模型 ID。不要照抄他人账户的模型 ID，是否可用取决于你的服务商账户权限。

### 测试失败时怎么处理

- **401 或未授权**：API Key 有误、已失效，或复制时多了空格。重新粘贴并保存。
- **404 或模型不存在**：接口地址或模型名称不对。优先从服务商控制台复制完整地址和模型 ID。
- **429 或余额/额度不足**：等待一会儿，或检查账户余额、并发和限额。
- **超时、连接失败**：检查网络、代理和服务商状态；可先换一个较快的模型测试。
- **测试成功但解析不完整**：模型只负责增强解析。请先确认 PDF 本身可复制文字；扫描件需要 OCR，复杂表格和图片也可能需要在个人资料页手动补充。

模型设置保存在本机资料库的 `profile.json` 中。删除程序、升级 EXE 或推送源码都不会主动上传该文件；若要换电脑，请自行以加密方式备份资料库，不要直接上传到公开仓库。
